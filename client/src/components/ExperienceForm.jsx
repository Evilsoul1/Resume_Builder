...
const ExperienceForm = ({data, onChange}) => {
    ...
    const generateJobDescription=async(index)=>{
        ...
        const promptData=`description:${experience.description}, position:${experience.position}, company:${experience.company}`;
        try {
            const response=await api.post("/api/ai/enhance-job-desc", {userContent:promptData}, {headers:{Authorization:token}})
                    ...
                    <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                        ...
                        <div className="space-y-2">
                            <div className='flex items-center justify-between'>
                                ...
                                <button disabled={generatingIndex===index || !experience.position || !experience.company} onClick={()=>generateJobDescription(index)} className='flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>
